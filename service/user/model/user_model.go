...
type (
	...
	// UserModelNoCache 不使用缓存
	UserModelNoCache interface {
		FindOneByNumber(number string) (*User, error)
	}
	...
	defaultUserModelNoCache struct {
		sqlx.SqlConn
		table string
	}
	...
	User struct {
		...
		Gender     sql.NullString `db:"gender"`   // 男｜女｜未公开
...
func NewUserModelNoCache(conn sqlx.SqlConn) UserModelNoCache {
	return &defaultUserModelNoCache{
		SqlConn: conn,
		table: 	"`user`",
	}
}
...
func (m *defaultUserModelNoCache) FindOneByNumber(number string) (*User, error) {
	var resp User
	query := fmt.Sprintf("select %s from %s where `number` = ? limit 1", userRows, m.table)
	err := m.QueryRow(&resp, query, number)
	switch err {
	case nil:
		return &resp, nil
	case sqlc.ErrNotFound:
